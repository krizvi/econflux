	...
	try {
		...
		if (kbResponse.data && Array.isArray(kbResponse.data)) {
			...
			sortedData.forEach((item, index) => {
				...
				// Create hyperlinked citation
				const hyperlinkCitation = `${udrBaseURL}/getDocumentPDF/?collectionName=${collectionName}&uniqueIdentifier=${uniqueIdentifier}`;

				// Add reference with file info
				references.push({
					collectionName: `${collectionName}`,
					uniqueIdentifier: `${uniqueIdentifier}`,
					url: `${hyperlinkCitation}`,
				});
...
export async function streamModelResponseWithCitations(opts: {
	baseURL: string;
	apiKey: string;
	udrBaseURL: string;
	prompt: string;
	...
	const { baseURL, apiKey, udrBaseURL, prompt, knowledgeBaseId } = opts;
	const response = await fetch(`${baseURL}/v1/vector_stores/${knowledgeBaseId}/search`, {
		headers: {
	...
	const fullText = formatKnowledgeBaseLLMPrompt(rawResponse, udrBaseURL);
	return {
		kind: "fullText",
		fullText,
	};
...
<script lang="ts">
	...
	// True while we want the loader *logically* visible
	$: showLoader = !$settings.disableStream && isLast && loading && emptyLoad;

	// Optional: 600ms grace period to avoid flashing on quick responses
	$: {
		if (showLoader) {
			if (loaderTimer == null) {
				loaderTimer = setTimeout(() => (showLoaderDeferred = true), 600);
			}
		} else {
			showLoaderDeferred = false;
			if (loaderTimer) {
...
{#if message.from === "assistant"}
	...
	>
		...
		{/if}
		<div
			class="relative min-h-[calc(2rem+theme(spacing[3.5])*2)] min-w-[60px] max-w-full break-words rounded-2xl border border-gray-100 bg-gradient-to-br from-gray-50 px-5 py-3.5 text-gray-600 prose-pre:my-2 dark:border-gray-800 dark:from-gray-800/40 dark:text-gray-300"
		>
	...
	{message.score
					? 'visible opacity-100'
					: 'visible opacity-85  transition-opacity duration-150 ease-in-out'}
			...
			<div class="flex w-full flex-row flex-nowrap">
				...
				{#if !loading && !editMode}
					<div
                        ...
                        top-3.5 z-10 h-max opacity-45 transition-opacity duration-150 ease-in-out max-md:transition-all md:bottom-0 md:group-hover:opacity-100 {isTapped ||
						isCopied
							? 'max-md:opacity-100'
							: ''}"
					>
						<div
							class="mx-auto flex flex-row flex-nowrap gap-2 transition-opacity duration-150 ease-in-out"
						>
							{#if downloadLink}
								<a
									class="md: rounded-lg border border-gray-100 bg-gray-100 p-1 text-xs text-gray-400 group-hover:block hover:text-gray-500 dark:border-gray-800 dark:bg-gray-800 dark:text-gray-400 dark:hover:text-gray-300 max-sm:!hidden"
							...
							{#if !readOnly}
								<button
									class="md: cursor-pointer rounded-lg border border-gray-100 bg-gray-100 p-1 text-xs text-gray-400 group-hover:block hover:text-gray-500 dark:border-gray-800 dark:bg-gray-800 dark:text-gray-400 dark:hover:text-gray-300 lg:-right-2"
									title="Branch"
									type="button"
	...
	>
		<svelte:fragment slot="childrenNav">
			{#if nChildren > 1 && $convTreeStore.editing === null}
				<div
					class="font-white group/navbranch -mt-1 ml-3.5 mr-auto flex h-6 w-fit select-none flex-row items-center justify-center gap-1 text-sm"
				>
					<button
						class="inline text-lg font-thin text-gray-400 hover:text-gray-800 disabled:pointer-events-none disabled:opacity-85 dark:text-gray-500 dark:hover:text-gray-200"
